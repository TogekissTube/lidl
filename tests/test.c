...
int main(int argc, char *argv[]) {
    ...
    elf_status_t read_status = elf_read_file(argv[1], buf);
    if (read_status != ELF_OK) {
    ...
    if (status == 0) {
        ...
        phdr_status_t phdr_status = phdr_parse_all(buf, &hdr, &phdrs, &phdr_count);
        if (phdr_status == PHDR_OK) {
            printf("[+] PHDR parsing SUCCESS!\n\n");
            
            for (size_t i = 0; i < phdr_count && i < 5; i++) {
                char flags[4];
                phdr_flags_to_string(phdrs[i].p_flags, flags, sizeof(flags));
                
                printf("PHDR[%zu]: Type=%s Flags=%s VAddr=0x%lx FileSize=%zu MemSize=%zu\n",
                       i,
                       phdr_type_to_string(phdrs[i].p_type),
                       flags,
                       phdrs[i].p_vaddr,
                       phdrs[i].p_filesz,
                       phdrs[i].p_memsz);
